    ...
    <div class="evaluate">
        <div class="flex wrap nav">
            <div class="flex__center__center nav__item" :class="loads.index==index?'active':''" v-for="(item,index) in classify" :key="index" @click="setIndex(index)">
                <span class="fs_24" :class="loads.index==index?'c_blue':'c_66'">{{item.sub_classify}}<text class="ml_15">{{item.num}}</text></span>
            </div>
        </div>
        <!-- 评价 -->
        <div class="evaluate-box">
            <div class="evaluate__item" v-for="(item,index) in list" :key="index">
                <div class="flex item__header">
                    ...
                    <div class="flex__null__around ml_20 flex_1">
                        <span class="fs_30 fs_24 c_33 fw_bold">{{item.nickname}}</span>
						<view class="flex flex__between mt_10">
							<view>
								<text class="fs_24 c_1a">评分</text>
								<text v-for="(item2,index2) in 5" :key="index2" class="iconfont icon-iospaw fs_24" :class="item2<item.star?'c_pur':'c_ed'"></text>
							</view>
							<text class="fs_24 c_98">{{item.create_time}}</text>
						</view>
                    </div>
                </div>
...
export default {
    ...
    methods:{
        loadList(page){
        	app.$vm.getList({
        		url:'Collage/getProductReviewList',
        		that: this,
        		data: this.loads,
        		loading: 1,
        		page
    ...
    onLoad(options){
		...
		app.$vm.getData({
			url:'Collage/getProductReviewData',
			that: this,
